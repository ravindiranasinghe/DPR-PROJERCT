...
# visualization part 
```{r}
df_viz <- cleaned_data %>% filter(year >= 2010 & year<= 2015 )
```

```{r}
ggplot(df_viz, aes(x = continent, y = pm25_concentration, fill = continent)) +
  geom_boxplot(outlier.alpha = 0.8) +
  labs(title = "PM2.5 Concentration by Continent (2010-2015)",
...
ggplot(df_viz, aes(y = reorder(country_name, pm25_concentration, FUN = median),
  ...
  coord_flip() +
  labs(title = "PM2.5 Concentration by Country (2010–2015)",
       x = "Country", y = "PM25 (µg/m³)") +
  theme_minimal()
...
df_viz %>%
  ...
  geom_line(size = 1) +
  geom_point(size = 1.5) +
  labs(title = "Yearly Mean PM2.5 (2010–2015)",
...
df_viz %>%
  ...
  labs(title = "Relationship between PM25 and NO2",
       x = "NO2 (µg/m³)", y = "PM25 (µg/m³)") +
  theme_minimal()
...
df_viz %>%
  group_by(country_name) %>%
  shapiro_test(pm25_concentration)
